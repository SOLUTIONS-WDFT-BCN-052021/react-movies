...
After that, create a new component called `Fave` that will eventually handle whether or not a movie is a user's favorite. The `Fave` component should return the following:
...
Now that you have a function that handles the user clicking a movie, connect it to the UI. In the `div` of `Fave`'s component, add a parameter of `onClick={handleClick}`.
...
#### Step 1: Import useState at the `Fave` Component
...
Back to the `Fave` component; declare a state for `isFave` and the default value should be `false`. This will set up the initial state of the component.

<details>
  <summary>Hint</summary>
  This will look like this:
  <code>const [isFave, setIsFave] = useState(false)</code>
</details>
...
Inside of the `handleClick()` method in the `Fave` component, use `setState()` to toggle the value of `isFave`. "Toggle" means you always want to set the new value to the opposite of the current value.

<details>
  <summary>Hint — one way to do this could be:</summary>
  <code>setIsFave(!isFave)</code>
</details>
...
<details>
  ...
  <code>const isFaveClass = (isFave) ? 'remove_from_queue' : 'add_to_queue'</code>

You can drop this in the <code>return</code> method.
<code>className={`film-row-fave ${isFaveClass}`}</code>
This checks the current <code>isFave</code> state for <code>true</code> or <code>false</code>.
...
In the `FilmListing` component, decalre a state called `filter` that has default value of `all`. This will set up the initial state of the component.
...
Inside of the `handleFilterClick()` method in the `FilmListing` component, use `setFilter()` to set the value of `filter` to the value passed to the event handler.
...
<details>
  ...
  This line in particular checks if the <code>filter</code> state is currently <code>'all'</code>; if it is, it sets the value to <code>is-active</code>. If it isn't, it does nothing: <code>className={`film-list-filter ${filter === 'all' ? 'is-active' : ''}`}</code>.
</details>
